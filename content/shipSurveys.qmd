...
To address this question, we need to collect data using traditional methods (shipboard visual surveys) and new methods (PAM-gliders) in the same region and at the same time. Data collected druing these concurrent surveys are needed to develop and test statistical methods needed to use PAM-glider data to assess marine mammal populations.

We will conduct concurrent shipboard/glider surveys off the U.S. West Coast (CalCurCEAS) and Hawaii (WHICEAS).
